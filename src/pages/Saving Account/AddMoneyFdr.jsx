...
import { makeDepositFdr } from "../../../api/admin";
const initialData = {
  date: new Date(),
  amount: 0,
  id: null,
...
function AddMoneyFdr({ id }) {
  const [formData, setFormData] = useState(initialData);
  const { mutate } = useMutationHook(makeDepositFdr, {
    key: [`fdr-account-${id}`, `fdr-transactions-${id}`],
    onSuccess: () => {
      toast.success("Done");
    },
    onError: (data) => {
